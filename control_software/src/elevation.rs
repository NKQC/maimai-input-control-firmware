...
//!   2. 虚拟摄像头: DirectShow 过滤器 DLL 要注册到机器级(HKLM\SOFTWARE\Classes, 会被任意
//!      消费端进程加载)。帧共享队列本身建在会话内的 `Local\` 命名空间, **不需要**管理员。
...
pub fn limitation_text() -> String {
    if is_elevated() {
        "当前以管理员运行: 固定 COM 口、虚拟摄像头安装/卸载均可用".to_string()
    } else {
        "当前为普通权限: 固定游戏 COM 口与虚拟摄像头安装/卸载需要管理员(帧共享本身不需要)"
            .to_string()
    }
}
