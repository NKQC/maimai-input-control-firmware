...
struct MediaSource;
using MediaSourceBase = winrt::implements<MediaSource, CBaseAttributes<IMFAttributes>, IMFMediaSourceEx, IMFGetService, IKsControl, IMFSampleAllocatorControl>;

struct MediaSource : MediaSourceBase
{
public:
	STDMETHODIMP QueryInterface(REFIID riid, void** ppv) noexcept override
	{
		auto hr = MediaSourceBase::QueryInterface(riid, ppv);
		WINTRACE(L"MediaSource::QueryInterface iid:%s hr:0x%08X object:%p", GUID_ToStringW(riid).c_str(), hr, ppv ? *ppv : nullptr);
		return hr;
	}
