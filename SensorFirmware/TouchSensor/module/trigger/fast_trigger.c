...
static uint16_t g_rise_permille_cfg = FAST_TRIG_RISE_PERMILLE_DEFAULT;
static uint16_t g_drop_permille_cfg = FAST_TRIG_DROP_PERMILLE_DEFAULT;
...
void fast_trigger_init(void)
{
    for (uint8_t i = 0; i < CAPSENSE_WIDGET_COUNT; ++i) {
        ...
        {
            uint16_t nth  = capsense_get_noise_th(i);
            uint16_t nnth = capsense_get_nnoise_th(i);
            uint16_t nz   = (nth > nnth) ? nth : nnth;
            uint32_t rise = (uint32_t)((ch->baseline * g_rise_permille_cfg) / 1000u + nz);
            uint32_t fall = (uint32_t)((ch->baseline * g_drop_permille_cfg) / 1000u + nz);
            ch->x_delta_rise = (rise > 0xFFFFu) ? 0xFFFFu : (uint16_t)rise;
            ch->x_delta_fall = (fall > 0xFFFFu) ? 0xFFFFu : (uint16_t)fall;
        }
        ch->need_reset = true;
    }
...
uint16_t fast_trigger_process(uint64_t now_ms, uint16_t base_status)
{
    ...
    for (_s_i = 0; _s_i < CAPSENSE_WIDGET_COUNT; ++_s_i) {
        ...
        if ((base_status >> _s_i) & 0x1u) {
            ...
            b = capsense_get_baseline(_s_i);
            d = capsense_get_diff(_s_i);
            ...
            }
            {
                ...
                if (latest_ts != 0 && oldest_ts != 0) {
                    ...
                    uint32_t thr = (diff > 0) ? (uint32_t)_s_ch->x_delta_rise : (uint32_t)_s_ch->x_delta_fall;
                    if ((diff >= 0 ? diff : -diff) > thr) {
...
{
    g_rise_permille_cfg = p;
    for (uint8_t i = 0; i < CAPSENSE_WIDGET_COUNT; ++i) {
        uint16_t nth  = capsense_get_noise_th(i);
        uint16_t nnth = capsense_get_nnoise_th(i);
        uint16_t nz   = (nth > nnth) ? nth : nnth;
        uint32_t rise = (uint32_t)((g_ch[i].baseline * g_rise_permille_cfg) / 1000u + nz);
        g_ch[i].x_delta_rise = (rise > 0xFFFFu) ? 0xFFFFu : (uint16_t)rise;
    }
}

void fast_trigger_set_drop_permille(uint16_t p)
{
    g_drop_permille_cfg = p;
    for (uint8_t i = 0; i < CAPSENSE_WIDGET_COUNT; ++i) {
        uint16_t nth  = capsense_get_noise_th(i);
        uint16_t nnth = capsense_get_nnoise_th(i);
        uint16_t nz   = (nth > nnth) ? nth : nnth;
        uint32_t fall = (uint32_t)((g_ch[i].baseline * g_drop_permille_cfg) / 1000u + nz);
        g_ch[i].x_delta_fall = (fall > 0xFFFFu) ? 0xFFFFu : (uint16_t)fall;
    }
}

uint16_t fast_trigger_get_rise_permille(void) { return g_rise_permille_cfg; }
uint16_t fast_trigger_get_drop_permille(void) { return g_drop_permille_cfg; }