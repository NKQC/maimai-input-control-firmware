...
| `0x02` | `LED_CONTROL` | R/W | 2 | `bit0={0,1}, bit1={0,1}` 其余保留 | 复位与LED控制 | `bit0=1` 触发软件重启；`bit1=1` 点亮；`bit1=0` 熄灭；其它位保留且忽略；读可返回最后写入值（重启后寄存器恢复默认） |
...
- `LED_CONTROL` 使用 `bit1` 控制 LED；`bit0=1` 触发软件复位；其它位保留，写入不会影响行为。复位后寄存器恢复默认值。
