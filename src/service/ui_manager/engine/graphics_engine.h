...
// 现代化UI颜色主题
#define COLOR_BG_DARK     0x1082  // 深色背景 #1a1a1a
#define COLOR_BG_CARD     0x2945  // 卡片背景 #2a2a2a
#define COLOR_BORDER      0x4208  // 边框颜色 #404040
#define COLOR_PRIMARY     0x06BF  // 主色调 #00d4ff
...
public:
    ...
    // 文本绘制（需要字库支持）
    void draw_text(const char* text, int16_t x, int16_t y, Color color, FontSize size = FontSize::MEDIUM);
    void draw_text_aligned(const char* text, const Rect& rect, Color color, 
                          TextAlign align = TextAlign::LEFT, FontSize size = FontSize::MEDIUM);
    
    // 中文文本绘制
    void draw_chinese_text(const char* utf8_text, int16_t x, int16_t y, Color color, FontSize size = FontSize::MEDIUM);
    ...
    // 工具函数
    int16_t get_text_width(const char* text, FontSize size = FontSize::MEDIUM) const;
    int16_t get_chinese_text_width(const char* utf8_text, FontSize size = FontSize::MEDIUM) const;
    int16_t get_font_height(FontSize size = FontSize::MEDIUM) const;
