...
PageJumpConstruct::PageJumpConstruct(const std::string& text, const std::string& target_page)
    : static_text_(text), target_page_(target_page), use_dynamic_text_(false) {
}

PageJumpConstruct::PageJumpConstruct(DynamicStringFunc text_func, const std::string& target_page)
...
void ConstructPage::render_to_page_template(PageTemplate& page_template) {
    ...
    for (size_t i = 0; i < constructs_.size() && i < 4; ++i) {
        ...
        if (construct) {
            LineType line_type = construct->is_interactive() ? LineType::MENU_JUMP : LineType::TEXT_ITEM;
            bool is_selected = (static_cast<int>(i) == selected_index_);
            
            LineConfig config(construct->get_display_text(), COLOR_TEXT_WHITE, LineAlign::LEFT);
            config.selected = is_selected;
            config.type = line_type;
...
void MenuInteractionSystem::register_page(const std::string& page_id, std::shared_ptr<PageTemplate> page) {
    if (page) {
        pages_[page_id] = page;
    }
}

void MenuInteractionSystem::unregister_page(const std::string& page_id) {
...
std::shared_ptr<PageTemplate> MenuInteractionSystem::get_page(const std::string& page_id) const {
    auto it = pages_.find(page_id);
    return (it != pages_.end()) ? it->second : nullptr;
}

bool MenuInteractionSystem::switch_to_page(const std::string& page_id) {
...
bool MenuInteractionSystem::handle_back_button() {
    ...
    std::string target_page = PageNavigationManager::getInstance().handle_back_navigation(has_interactive);
