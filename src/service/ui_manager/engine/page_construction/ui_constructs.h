...
public:
    // 构造函数 - 静态文本
    PageJumpConstruct(const std::string& text, const std::string& target_page);
    
    // 动态文本构造函数
    PageJumpConstruct(DynamicStringFunc text_func, const std::string& target_page);
    ...
    // 获取目标页面
    std::string get_target_page() const { return target_page_; }
    
private:
    std::string static_text_;
    DynamicStringFunc dynamic_text_func_;
    std::string target_page_;
...
public:
    ...
    // 页面切换
    bool switch_to_page(const std::string& page_id);
...
private:
    ...
    std::map<std::string, std::shared_ptr<PageTemplate>> pages_;
    std::string current_page_id_ = "main";
};
