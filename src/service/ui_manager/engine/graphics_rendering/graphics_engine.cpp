...
void GraphicsEngine::draw_text(const char* text, int16_t x, int16_t y, Color color) {
    if (!text) return;
    
    int16_t cursor_x = x;
    int16_t font_height = get_font_height();
    ...
    while (*p) {
        ...
        if ((*p & 0x80) == 0) {
            ...
            draw_character(single_char, cursor_x, y, color);
            cursor_x += get_character_width(single_char);
            p++;
        } else if ((*p & 0xE0) == 0xC0) {
            // 2字节UTF-8字符 (110xxxxx 10xxxxxx)
            if ((*(p+1) & 0xC0) == 0x80) {
                char utf8_char[3] = {*p, *(p+1), '\0'};
                draw_character(utf8_char, cursor_x, y, color);
                cursor_x += get_character_width(utf8_char);
                p += 2;
            } else {
                p++; // 跳过无效字符
            }
        } else if ((*p & 0xF0) == 0xE0) {
            // 3字节UTF-8字符 (1110xxxx 10xxxxxx 10xxxxxx) - 中文字符
            if ((*(p+1) & 0xC0) == 0x80 && (*(p+2) & 0xC0) == 0x80) {
                char utf8_char[4] = {*p, *(p+1), *(p+2), '\0'};
                draw_character(utf8_char, cursor_x, y, color);
                cursor_x += get_character_width(utf8_char);
                p += 3;
            } else {
            ...
            if ((*(p+1) & 0xC0) == 0x80 && (*(p+2) & 0xC0) == 0x80 && (*(p+3) & 0xC0) == 0x80) {
                char utf8_char[5] = {*p, *(p+1), *(p+2), *(p+3), '\0'};
                draw_character(utf8_char, cursor_x, y, color);
                cursor_x += get_character_width(utf8_char);
                p += 4;
            } else {
...
void GraphicsEngine::draw_text_aligned(const char* text, const Rect& rect, Color color, TextAlign align) {
    if (!text) return;
    
    int16_t text_width = get_text_width(text);
    int16_t text_height = get_font_height();
...
void GraphicsEngine::draw_chinese_text(const char* utf8_text, int16_t x, int16_t y, Color color) {
    if (!utf8_text) return;
...
void GraphicsEngine::draw_chinese_text_aligned(const char* utf8_text, const Rect& rect, Color color, TextAlign align) {
    if (!utf8_text) return;
    
    int16_t text_width = get_chinese_text_width(utf8_text);
    int16_t text_height = get_font_height();
    ...
    draw_chinese_text(utf8_text, x, y, color);
}
...
void GraphicsEngine::draw_button(const Rect& rect, const char* text, Color bg_color, Color text_color, bool pressed) {
    ...
    if (text) {
        draw_text_aligned(text, rect, text_color, TextAlign::CENTER);
    }
}
...
void GraphicsEngine::draw_list_item(const Rect& rect, const char* text, const char* icon, bool selected) {
    ...
    // 绘制图标（如果有）
    if (icon && *icon) {
        draw_text(icon, x_offset, rect.y + (rect.height - get_font_height()) / 2, text_color);
        x_offset += 20;
    }
    
    // 绘制文本
    if (text) {
        draw_chinese_text(text, x_offset, rect.y + (rect.height - get_font_height()) / 2, text_color);
    }
}
...
// 工具函数实现
int16_t GraphicsEngine::get_text_width(const char* text) const {
    if (!text) return 0;
    
    int16_t width = 0;
    while (*text) {
        char single_char[2] = {*text, '\0'};
        width += get_character_width(single_char);
        text++;
    }
    return width;
}

int16_t GraphicsEngine::get_chinese_text_width(const char* utf8_text) const {
    ...
    while (*p) {
        if ((*p & 0xE0) == 0xE0 && (*(p+1) & 0x80) == 0x80 && (*(p+2) & 0x80) == 0x80) {
            // 3字节UTF-8字符（中文）
            char utf8_char[4] = {*p, *(p+1), *(p+2), '\0'};
            width += get_character_width(utf8_char);
            p += 3;
        } else if (*p >= 32 && *p <= 126) {
            // ASCII字符
            char single_char[2] = {*p, '\0'};
...
int16_t GraphicsEngine::get_font_height() const {
    // 使用标准字符高度（中文字符标准）
    return 14;
}
...
// 字符绘制相关函数实现
void GraphicsEngine::draw_character(const char* utf8_char, int16_t x, int16_t y, Color color) {
    if (!utf8_char) return;
    
    // 使用统一的字符查找接口
    FontSearchResult result = FontData::find_character(utf8_char);
    
    if (result.found) {
        // 找到字符，绘制点阵
        draw_char_bitmap(result.bitmap_data, result.width, result.height, x, y, color);
    } else {
        // 使用默认方框字符
        static uint8_t default_box_buffer[28]; // 14x14需要28字节
        FontData::create_default_box_char(default_box_buffer);
        draw_char_bitmap(default_box_buffer, 14, 14, x, y, color);
    }
}

int16_t GraphicsEngine::get_character_width(const char* utf8_char) const {
    if (!utf8_char) return 0;
...
void GraphicsEngine::draw_char_bitmap(const uint8_t* bitmap_data, uint8_t width, uint8_t height, 
    ...
    for (uint8_t row = 0; row < height; row++) {
        for (uint8_t col = 0; col < width; col++) {
            // 计算当前像素在当前行中的字节索引和位偏移
            uint8_t byte_in_row = col / 8;
            uint8_t bit_offset = 7 - (col % 8);  // MSB在左
            
            // 计算在整个位图数据中的字节索引
            uint16_t byte_index = row * bytes_per_row + byte_in_row;
            
            // 检查该位是否设置
            if (pgm_read_byte(&bitmap_data[byte_index]) & (1 << bit_offset)) {
