...
- 目标灵敏度通过测量CDC值分布+实际触发情况+目标灵敏度步进扫描校准
- 目标灵敏度并不等同于实际灵敏度 该灵敏度设置本质是自动测量并设置CDC电压偏置 属于物理电路的前置操作
- 这种操作具有极高的重复精度和稳定性 且不影响读取CDC精度 几乎不受外界影响
- 目标灵敏度可设置范围为 -10 ~ 10  (最低灵敏目标 ~ 最高灵敏目标)
- 真正的灵敏度影响在每个通道独立的CDC值后处理 默认设置为15 峰值为0
...
1. **GPIO -> Keyboard 映射**：将物理按钮（连接到GPIO引脚）直接映射为键盘按键
   - 适用于：外接物理按钮、开关等硬件输入设备
   - 特点：轮询次数在10K以上 响应低于1ms

2. **TouchArea -> Keyboard 映射**：将触摸屏上的特定区域映射为键盘按键
   - 适用于：Mai2触控屏操作，可设置长按时间和触发模式
   - 特点：支持长按触发、单次触发等设置
...
| GPIOB0   | 8           | BUTTON9 |
| GPIOB1   | 3           | BUTTONA |
| GPIOB2   | Enter       | BUTTONB |
| GPIOB3   | Space       | BUTTONC |
...
**触发模式说明：**
- **单次触发**：达到触发条件后按键只按一下，需要松开重新触发
- **持续触发**：达到触发条件后按键一直按下，直到不满足条件才松开
